...
# Add the mcp-x-server directory to the path
server_dir = Path(__file__).parent / "mcp-x-server"
if str(server_dir) not in sys.path:
    sys.path.append(str(server_dir))

# Import the main entry point from the server
...
if __name__ == "__main__":
    ...
    if "X_DATA_DIR" not in os.environ:
        default_data_dir = os.path.join(os.path.dirname(__file__), "x_data")
